<?php
	...
	$qu_customers_nutrition_diets_sel = "SELECT * FROM  `customers_nutrition_diets_supplement` WHERE `nutrition_plan_id` = $nutrition_plan_id";
	...
	$IAM_ARRAY['success'] = true;
		while( $ARRAY_SRC = mysqli_fetch_assoc($qu_customers_nutrition_diets_EXE) ){
			...
			$IAM_ARRAY['result'][] = array( "supplement_name" => $ARRAY_SRC['supplement_name'], 
		...
		$IAM_ARRAY['success'] = false;
		$IAM_ARRAY['result'] = array(  "result" => "No Supplements Found" );
		echo json_encode($IAM_ARRAY, JSON_FORCE_OBJECT);
		die();
	}
