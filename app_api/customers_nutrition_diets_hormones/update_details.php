...
if( isset($_POST['record_id']) ){
	...
	if( isset($_POST['hormone_id']) ){
		...
		$qqq = $qqq."`hormone_id` = '".$hormone_id."', ";
	}
	$hormone_name = "";
	if( isset($_POST['hormone_name']) ){
		$hormone_name = test_inputs( $_POST['hormone_name'] );
		$qqq = $qqq."`hormone_name` = '".$hormone_name."', ";
	}
	$comments = "";
	...
	if( $qqq != "" ){
		$qqq =  substr($qqq, 0, -2);
		$qu_customers_nutrition_diets_hormones_updt = "UPDATE  `customers_nutrition_diets_hormones` SET ".$qqq."  WHERE `record_id` = $record_id;";

		if(mysqli_query($KONN, $qu_customers_nutrition_diets_hormones_updt)){
