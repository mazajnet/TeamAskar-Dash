...
$IAM_ARRAY['success'] = false;
	...
	$qu_customers_private_plans_days_sel = "SELECT * FROM  `customers_private_plans_days` WHERE `day_id` = $day_id";
	$qu_customers_private_plans_days_EXE = mysqli_query($KONN, $qu_customers_private_plans_days_sel);

	if(mysqli_num_rows($qu_customers_private_plans_days_EXE) == 1){
	
		$ARRAY_SRC = mysqli_fetch_assoc($qu_customers_private_plans_days_EXE);
